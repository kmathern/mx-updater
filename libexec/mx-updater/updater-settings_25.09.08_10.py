...
class SettingsEditorDialog(QDialog):
    ...
    def update_setting_dialog(self, key, value):
        ...
        match key:
            ...
            case 'start_at_login':
                self.settings[key] = new_value
                self.start_at_login_checkbox.setChecked(new_value)
...
when no updates are available."""))
        ...
        checkbox_label = _("close terminal window automatically after idle time")
...
system updates are available."""))
       ...
       # set connection
        ...
        self.start_at_login_checkbox = QCheckBox(_("start MX Updater at login"))
        self.start_at_login_checkbox.setToolTip(
        _("""Select this to launch MX Updater automatically at login"""))

        if self.settings.get("start_at_login"):
            self.start_at_login_checkbox.setChecked(True)
        else:
            self.start_at_login_checkbox.setChecked(False)
        # set connection
        self.start_at_login_checkbox.toggled.connect(
            lambda checked:
            self.on_start_at_login_checkbox_toggled(checked))
...
Untick this box or run "MX Updater" from the menu to make the icon visible again."""))
        ...
        other_options_layout.addWidget(self.start_at_login_checkbox)
    ...
    def on_start_at_login_checkbox_toggled(self, checked):
        # save start_at_login selection into settings dict
        self.settings["start_at_login"] = checked
        self.qsettings.setValue("Settings/start_at_login", checked)
        print(f"toggled start_at_login: {checked}")
