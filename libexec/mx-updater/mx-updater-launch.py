...
if args.autostart:
    ...
    start_at_login_enabled = QSettings("MX-Linux", "mx-updater").value(
        "Settings/start_at_login", "true").lower() in ("true", "on", "1")
    if not start_at_login_enabled:
        logger.info("MX Updater autostart is disabled in settings, exiting")
        sys.exit(1)    
...
try:
    saved_delay = settings.value('AutoStart/delay', type=int)
...
if args.wait:
    delay = args.wait
    if delay < 0:
        logger.debug("AutoStart/delay -w/wait '%s' negative value ignored - using default", delay)
        delay = default_delay
    delay = max(0, delay)
    logger.debug("AutoStart delay saved as '%s'", delay)
    settings.setValue('AutoStart/delay', delay)
    settings.sync()
